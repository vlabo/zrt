...
var registers: *volatile Registers = @intToPtr(*volatile Registers, mmio.UART_REGISTERS);
...
pub fn setup() bool {
    ...
    registers.interupt_clear_reg = 0x7FF;
    registers.integer_bound_rate_divisor = 2;
    registers.fractal_bound_rate_divisor = 0xB;
    registers.interupt_mask_clear_reg = 0b11 << 5;
    registers.control_reg = 0x301;
    return true;
}

pub fn write_char(c: u8) bool {
    while ((registers.flag_reg & 0x20) != 0) {
        cpu.nop();
    }
    registers.data_reg = c;
    return true;
}

pub fn read_char() u8 {
    while ((registers.flag_reg & 0x10) != 0) {
        asm volatile ("nop");
    }

    var c: u8 = @intCast(u8, registers.data_reg);
    if (c == '\r') {
        return '\n';
    }
